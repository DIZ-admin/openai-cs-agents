...
def verify_services():
    ...
    print(f"Backend ({BACKEND_URL}): {'✓ HEALTHY' if backend_health else '✗ UNHEALTHY'}")
    ...
    print(f"Frontend ({FRONTEND_URL}): {'✓ RUNNING' if frontend_running else '✗ NOT RUNNING'}")
...
class TestAppointmentBooking:
    ...
    def test_appointment_booking_flow(self, test_start_time):
        ...
        assert any(word in response_text for word in ["architekt", "architect", "specialist", "available"]), \
...
class TestPerformance:
    ...
    def test_response_time(self, test_start_time):
        ...
        print(f"✓ Performance: {'EXCELLENT' if duration < 3 else 'GOOD' if duration < 5 else 'ACCEPTABLE'}")
        print("\n✅ TEST 7 PASSED")
...
class TestMultiTurnConversation:
    ...
    def test_context_preservation(self, test_start_time):
        ...
        assert response2["context"]["inquiry_id"] == response1["context"]["inquiry_id"], \
...
class TestFAQAgentVectorStore:
    ...
    def test_company_contact_info(self, test_start_time):
        ...
        message = "I need the contact details for ERNI Gruppe timber construction company"
        print(f"\n📤 Query: '{message}'")
        response = send_chat_message(message)
        ...
        assert "guggibadstrasse" in content.lower() or "schongau" in content.lower() or "6288" in content, \
    ...
    def test_certifications(self, test_start_time):
        ...
        print(f"✓ Agent: {response['current_agent']}")
        print(f"✓ Found certifications in response")
        print(f"✓ Preview: {content[:150]}...")
        print("\n✅ TEST 9.2 PASSED")
    ...
    def test_divisions_services(self, test_start_time):
        ...
        # Check for divisions
        divisions = ["planung", "holzbau", "spenglerei", "ausbau", "realisation", "agrar"]
        ...
        print(f"✓ Agent: {response['current_agent']}")
        print(f"✓ Found divisions: {', '.join(found_divisions)} ({len(found_divisions)}/6)")
        print(f"✓ Preview: {content[:150]}...")
        print("\n✅ TEST 9.3 PASSED")
    ...
    def test_wood_advantages(self, test_start_time):
        ...
        advantages = ["ökologisch", "ecological", "co2", "nachhaltig", "sustainable", "raumklima", "climate"]
