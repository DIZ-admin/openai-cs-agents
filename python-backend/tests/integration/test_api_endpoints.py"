...
class TestAPIEndpoints:
    ...
    def test_chat_endpoint_guardrail_triggered(self, mock_store, mock_runner, client):
        ...
        mock_guardrail_output = GuardrailFunctionOutput(
            output_info=RelevanceOutput(reasoning="Not related to construction", is_relevant=False),
            tripwire_triggered=True
        )
        # Create a mock guardrail result object using the actual guardrail
        mock_guardrail_result = MagicMock()
        mock_guardrail_result.guardrail = relevance_guardrail  # Use actual guardrail for proper identification
        ...
        assert "Sorry, I can only answer questions related to building and construction" in data["messages"][0]["content"]
    ...
    def test_chat_endpoint_agent_handoff(self, mock_store, mock_runner, client):
        ...
        mock_raw_item.content = [{"type": "text", "text": "I can help with cost estimation"}]
        ...
        request_data = {
            "message": "How much would a house cost?"
        }
    ...
    def test_cors_headers(self, mock_store, mock_runner, client):
        ...
        response = client.post("/chat", json={"message": "test"}, headers={"Origin": "http://localhost:3000"})
    ...
    def test_chat_endpoint_context_updates(self, mock_store, mock_runner, client):
        ...
        updated_context = BuildingProjectContext(
            customer_name="Hans Müller",
        ...
        context_update_events = [e for e in data["events"] if e["type"] == "context_update"]
        assert len(context_update_events) >= 0  # May or may not have context updates depending on mock
