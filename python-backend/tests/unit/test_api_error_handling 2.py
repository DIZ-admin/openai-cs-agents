...
class TestAPIErrorHandling:
    ...
    def test_openai_rate_limit_error(self, mock_runner, client, auth_headers):
        ...
        # Mock Runner to raise rate limit error
        mock_runner.return_value.run = AsyncMock(
    ...
    @patch("api.get_session_manager")
    def test_session_manager_error(self, mock_session_manager, client, auth_headers):
        """Test handling of session manager errors."""
        # Mock session manager to raise error
        mock_session_manager.return_value.get_session = MagicMock(
    ...
    @patch("api.SQLiteSession")
    def test_sqlite_session_error(self, mock_sqlite, client, auth_headers):
        """Test handling of SQLite session errors."""
        # Mock SQLiteSession to raise error
        mock_sqlite.side_effect = Exception("SQLite error")
