...
from main import RelevanceOutput, relevance_guardrail
...
async def relevance_guardrail_test(
    ...
    building_keywords = [
        "build", "construction", "house", "project", "erni", "estimate",
        "consultation", "architect", "timber", "wood", "planning", "renovation",
        "holzbau", "bauleiter", "specialist", "availability", "wooden", "timeline",
        "minergie", "certification", "status"
    ]

    conversational_keywords = [
        "hi", "hello", "thank", "yes", "no", "ok", "goodbye", "thanks"
    ]
    ...
    elif any(text_content.strip().lower() == keyword for keyword in conversational_keywords):
        is_relevant = True
    # Check for partial conversational matches in short phrases (<=15 chars)
    elif len(text_content.strip()) <= 15 and any(keyword in text_content for keyword in conversational_keywords):
        is_relevant = True
    ...
    output_info = type('OutputInfo', (), {
        ...
        'reasoning': "Test relevance check"
    })()
...
class TestRelevanceGuardrail:
    ...
    async def test_relevance_guardrail_allows_building_related_input(
        ...
        with patch("main.Runner.run") as mock_runner:
            ...
            for input_text in building_inputs:
                result = await relevance_guardrail_test(mock_context, mock_agent, input_text)
    ...
    async def test_relevance_guardrail_allows_conversational_input(
        ...
        with patch("main.Runner.run") as mock_runner:
            ...
            mock_result = MagicMock()
            mock_result.final_output_as.return_value = RelevanceOutput(
                reasoning="Conversational input is acceptable",
                is_relevant=True
            )
            mock_runner.return_value = mock_result
    ...
    async def test_relevance_guardrail_with_list_input(self, mock_context, mock_agent):
        ...
        with patch("main.Runner.run") as mock_runner:
            mock_result = MagicMock()
            mock_result.final_output_as.return_value = RelevanceOutput(
                reasoning="Building-related conversation",
                is_relevant=True
            )
            mock_runner.return_value = mock_result

            result = await relevance_guardrail_test(mock_context, mock_agent, message_list)
